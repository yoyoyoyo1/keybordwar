...
public interface ShareInfoDao extends JpaRepository<ShareInfo,String> {
    ...
    @Query(value = "select Share.userId,Share.id,User.nickname,SharePicture.img,User.image,Share.content,Share.comments,Share.likes,Share.forwards,Share.createdAt,10 AS likeInfo from Share left join User on Share.userId = User.id    " +
            "left join SharePicture on Share.id=SharePicture.shareId where Share.userId in ?1 ORDER BY createdAt DESC LIMIT 5",nativeQuery = true)
    ...
    @Query(value = "select Share.userId,Share.id,User.nickname,SharePicture.img,User.image,Share.content,Share.comments,Share.likes,Share.forwards,Share.createdAt,10 AS likeInfo from Share left join User on Share.userId = User.id    " +
            "left join SharePicture on Share.id=SharePicture.shareId where Share.userId in ?1 ORDER BY createdAt DESC LIMIT ?2,5",nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM Share where Share.userId in ?1",nativeQuery = true)
    public int countOneFollowingShareNum(List<Integer> following);
